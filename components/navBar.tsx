...
export function NavBar() {
  ...
  return (
    <div className="w-[90%] m-auto flex justify-between py-[21px] items-center">
      ...
      <div className="hidden gap-[clamp(30px,3vw,50px)] items-center lg:flex">
        {items.map((item, index) => (
          <Link
            className="text-[black] font-normal text-[22px] last:bg-[#1FC28B] last:p-[10px] last:text-[white] last:rounded-[8px]"
            href={item.link}
            key={index}
          >
            {item.name}
